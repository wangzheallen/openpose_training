...
def writeLMDB(datasets, lmdb_path, validation):
    ...
    for count in range(numSample):
        ...
        if "MPI" in data[idx]['dataset']:
            path_header = mpiDatasetFolder + 'images/'
        elif "COCO" in data[idx]['dataset']:
            path_header = cocoDatasetFolder + 'images/'
        ...
        if "COCO_val" in data[idx]['dataset']:
            ...
            #print img_idx
            mask_miss = cv2.imread(mpiDatasetFolder + 'masks/mask_' + img_idx + 'jpg', 0)
            #mask_all = mask_miss
...
if __name__ == "__main__":
    #writeLMDB(['MPI'], mpiSavingFolder, 1)
    writeLMDB(['COCO'], cocoSavingFolder, 1)
