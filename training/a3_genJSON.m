function a3_genJSON(dataset)
    ...
    if(strcmp(dataset, 'COCO'))
        ...
        for mode = 0:1
            ...
            trainIdx = 1:1:size(matAnnotations,2);
            ...
            for i = trainIdx
                numPeople = length(matAnnotations(i).annorect);
                ...
                for p = 1:numPeople
                    ...
                    if matAnnotations(i).annorect(p).num_keypoints < 5 || matAnnotations(i).annorect(p).area < 32*32
                        continue;
                    end
                    % skip this person if the distance to exiting person is too small
                    person_center = [matAnnotations(i).annorect(p).bbox(1)+matAnnotations(i).annorect(p).bbox(3)/2, ...
                                     matAnnotations(i).annorect(p).bbox(2)+matAnnotations(i).annorect(p).bbox(4)/2];
                    flag = 0;
                    ...
                    anno = matAnnotations(i).annorect(p).keypoints;

                    % set image path
                    if mode == 0
                        joint_all(count).img_paths = sprintf('train2014/COCO_train2014_%012d.jpg', matAnnotations(i).image_id);
                    else
                        joint_all(count).img_paths = sprintf('val2014/COCO_val2014_%012d.jpg', matAnnotations(i).image_id);
                    end
                    %joint_all(count).img_paths = RELEASE(i).image_id;
                    %[h, w, ~] = size(imread([datasetFolder, '/images/', joint_all(count).img_paths]));
                    joint_all(count).img_width = w;
                    joint_all(count).img_height = h;
                    joint_all(count).objpos = person_center;
                    joint_all(count).image_id = matAnnotations(i).image_id;
                    joint_all(count).bbox = matAnnotations(i).annorect(p).bbox;
                    joint_all(count).segment_area = matAnnotations(i).annorect(p).area;
                    joint_all(count).num_keypoints = matAnnotations(i).annorect(p).num_keypoints;
                    ...
                    % set scale
                    joint_all(count).scale_provided = matAnnotations(i).annorect(p).bbox(4)/368;
                    ...
                    for op = 1:numPeople
                        if op == p || matAnnotations(i).annorect(op).num_keypoints == 0
                            continue; 
                        end
                        anno = matAnnotations(i).annorect(op).keypoints;

                        joint_all(count).scale_provided_other(count_other) = matAnnotations(i).annorect(op).bbox(4)/368;
                        %joint_all(count).scale_provided_other(count_other) = RELEASE(i).annorect(op).area;
                        joint_all(count).objpos_other{count_other} = [...
                            matAnnotations(i).annorect(op).bbox(1) + matAnnotations(i).annorect(op).bbox(3)/2, ...
                            matAnnotations(i).annorect(op).bbox(2) + matAnnotations(i).annorect(op).bbox(4)/2 ...
                        ];
                        joint_all(count).bbox_other{count_other} = matAnnotations(i).annorect(op).bbox;
                        joint_all(count).segment_area_other(count_other) = matAnnotations(i).annorect(op).area;
                        joint_all(count).num_keypoints_other(count_other) = matAnnotations(i).annorect(op).num_keypoints;
                    ...
                    if(makeFigure) % visualizing to debug
                        imshow([datasetFolder, 'images/', joint_all(count).img_paths]);
                        ...
                        max(matAnnotations(i).annorect(p).bbox(3), matAnnotations(i).annorect(p).bbox(4))
                        sqrt(joint_all(count).scale_provided)
                        rectangle('Position',[joint_all(count).objpos(1)-rect_size, ...
                                              joint_all(count).objpos(2)-rect_size, ...
                                              rect_size*2, ...
                                              rect_size*2], ...
                                  'EdgeColor','b')
                        pause;
                    ...
                    prev_center = [prev_center; joint_all(count).objpos max(matAnnotations(i).annorect(p).bbox(3), matAnnotations(i).annorect(p).bbox(4))];
                    count = count + 1;
