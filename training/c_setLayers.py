...
import os
caffeFolder =  '../data/caffe_train/'
lmdbFolder = '../data/lmdb_trainVal/'
directory = './COCO_exp_caffe/pose56/exp22/'
caffeModelsFolder = os.path.join(directory, 'model')
pretrainedModelPath = '../data/vgg/VGG_ILSVRC_19_layers.caffemodel'
...
caffeFolder =  os.path.abspath(caffeFolder)
lmdbFolder = os.path.abspath(lmdbFolder)
directory = os.path.abspath(directory)
caffeModelsFolder = os.path.abspath(caffeModelsFolder)
pretrainedModelPath = os.path.abspath(pretrainedModelPath)
# # Printing those paths
# print caffeFolder
# print lmdbFolder
# print directory
# print caffeModelsFolder
...
def writePrototxts(dataFolder, sub_dir, batch_size, layername, kernel, stride, outCH, transform_param_in, base_lr, folder_name, label_name='label_1st', batchnorm=0, lr_mult_distro=[1,1,1], new=0):
    ...
    if new == 6:
        ...
        with open('%s/pose_train_test.prototxt' % sub_dir, 'w') as f:
            ...
            str_to_write = setLayers_twoBranches(dataFolder, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False, batchnorm=batchnorm, lr_mult_distro=lr_mult_distro)
...
def getBash():
    return ('#!/usr/bin/env sh\n' + caffeFolder + '/build/tools/caffe train --solver=pose_solver.prototxt --gpu=$1 \
--weights=' + pretrainedModelPath + ' 2>&1 | tee ./output.txt')
    ...
    if(exp == 1):
        ...
        for nc in range(0,1):
            ...
            if not os.path.exists(sub_dir):
                os.makedirs(sub_dir)
            if not os.path.exists(caffeModelsFolder): # for storing caffe models
                os.makedirs(caffeModelsFolder)

            label_name = ['label_vec', 'label_heat', 'vec_weight', 'heat_weight', 'vec_temp', 'heat_temp']
            writePrototxts(lmdbFolder, sub_dir, batch_size, layername, kernel, stride, outCH, transform_param, base_lr, caffeModelsFolder, label_name, 0, lr_mult_distro, 6)
